...
import {
  X, ShoppingCart, Minus, Plus, CreditCard, Smartphone, Truck,
          ...
          >
            ...
            <div className="px-6 pb-8 pt-2">
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <Button size="icon" variant="ghost" className="rounded-full h-8 w-8" onClick={onClose}>
                  <X className="w-4 h-4" />
                </Button>
              </div>
              ...
              {step === "cart" && (
                <div className="space-y-4">
                  {cart.length === 0 ? (
                    ...
                    <>
                      {cart.map((item) => (
                        <div key={item.id} className="flex items-center gap-3 p-3 rounded-2xl bg-card/80 backdrop-blur-sm border border-border/40">
                          {item.image_url && (
              ...
              {step === "info" && (
                <div className="space-y-4">
                  <div className="space-y-1">
                    ...
                    <Input value={buyerName} onChange={(e) => setBuyerName(e.target.value)} placeholder="Juan dela Cruz" className="rounded-xl h-11" />
                  </div>
                  <div className="space-y-1">
                    <Label className="text-xs">Phone Number</Label>
                    <Input value={buyerPhone} onChange={(e) => setBuyerPhone(e.target.value)} placeholder="09XX XXX XXXX" className="rounded-xl h-11" />
                  </div>
                  <div className="space-y-1">
                    <Label className="text-xs">Delivery Location</Label>
                    <Input value={buyerLocation} onChange={(e) => setBuyerLocation(e.target.value)} placeholder="123 Main St, Cebu City" className="rounded-xl h-11" />
                  </div>
              ...
              {step === "payment" && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      <Smartphone className="w-6 h-6 mx-auto mb-1 text-[#007DFE]" />
                      <p className="text-xs font-semibold">GCash</p>
                    </button>
                    <button
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      className="flex-1 rounded-xl gradient-primary text-primary-foreground"
                      disabled={submitting || (paymentMethod === "gcash" && !gcashQrUrl)}
                    >
              ...
              {step === "done" && (
                <div className="text-center py-8 space-y-4">
                  ...
                  <Button onClick={onClose} className="rounded-xl gradient-primary text-primary-foreground">
                    Done
                  </Button>
                </div>
              )}
