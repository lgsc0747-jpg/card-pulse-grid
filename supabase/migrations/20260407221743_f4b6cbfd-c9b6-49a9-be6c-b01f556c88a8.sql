...
CREATE FUNCTION public.get_public_persona(p_user_id uuid, p_slug text DEFAULT NULL::text)
 RETURNS TABLE(id uuid, slug text, label text, is_active boolean, is_private boolean, has_pin boolean, require_contact_exchange boolean, display_name text, headline text, bio text, avatar_url text, email_public text, phone text, location text, website text, linkedin_url text, github_url text, cv_url text, accent_color text, secondary_color text, tertiary_color text, text_color text, landing_bg_color text, background_preset text, background_image_url text, card_bg_image_url text, card_bg_size text, glass_opacity numeric, font_family text, text_alignment text, card_blur numeric, card_texture text, availability_status text, work_mode text, show_availability boolean, show_location boolean, user_id uuid, gcash_qr_url text, border_radius integer, shadow_preset text, card_animation text)
 LANGUAGE sql
